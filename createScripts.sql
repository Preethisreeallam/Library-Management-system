...
CREATE TABLE archive_book_loaned_details(
		loan_id INT PRIMARY KEY NOT NULL GENERATED ALWAYS AS IDENTITY,
		book_id INT NOT NULL ,
		branch_id INT NOT NULL ,
		student_id VARCHAR(10) NOT NULL,
		...
		CONSTRAINT fk_ar_book_loaned_details_book_id FOREIGN KEY(book_id) REFERENCES book_details(book_id) ON UPDATE RESTRICT ON DELETE RESTRICT,
		CONSTRAINT fk_ar_book_loaned_details_branch_id FOREIGN KEY(branch_id) REFERENCES library_branch_details(branch_id) ON UPDATE RESTRICT ON DELETE RESTRICT,
		CONSTRAINT fk_ar_book_loaned_details_student_id FOREIGN KEY(student_id) REFERENCES student_details(student_id) ON UPDATE RESTRICT ON DELETE RESTRICT
);